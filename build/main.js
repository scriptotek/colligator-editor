  ...
  methods: {
    ...
    notFound: function notFound() {
      ...
      Documents.cannotFindCover({ id: this.doc.id }, {}).then(function (response) {
        ...
        // error callback
        _this2.errors = ['Save failed because of network or server issues.'];
        console.log(response);
        if (response.status === 422) {
          _this2.errors = Object.keys(response.body).map(function (k) {
    ...
    submit: function submit() {
      ...
      Documents.saveCover({ id: this.doc.id }, { url: this.url }).then(function (response) {
        ...
        // error callback
        _this3.errors = ['Save failed because of network or server issues.'];
        console.log(response);
        if (response.status === 422) {
          _this3.errors = Object.keys(response.body).map(function (k) {
            return response.body[k][0];
          });
